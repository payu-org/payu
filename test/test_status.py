...
@pytest.fixture
def running_job_file(tmp_path, request):
...
@pytest.mark.parametrize(
    "archive_job_files,running_job_file,queued_job_file,latest_file,all_files",
    [
    ...
    indirect=["archive_job_files", "queued_job_file", "running_job_file"]
)
def test_get_job_file_list(tmp_path, archive_job_files, queued_job_file,
                           running_job_file, latest_file, all_files):
...
@pytest.mark.parametrize(
    "archive_job_files,running_job_file,queued_job_file,run_number,expected_file",
    [
...
def test_get_job_file_list_selected_run(tmp_path, queued_job_file,
                                        running_job_file, archive_job_files,
...
@pytest.mark.parametrize(
    "archive_job_files,running_job_file,queued_job_file,expected",
    [
        (
            False, False, True,
            {
    ...
    indirect=["archive_job_files", "running_job_file", "queued_job_file"]
)
def test_query_job_info(tmp_path, archive_job_files, running_job_file,
    ...
    if 'runs' in latest_data:
        for payu_jobs in latest_data['runs'].values():
            if 'run' in payu_jobs:
