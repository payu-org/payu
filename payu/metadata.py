...
PLEASE_UPDATE_COMMENT = "---- User please update. ----"
...
class Metadata:
    ...
    def update_file(self,
        ...
        metadata, last_auto_index = arrange_metadata(metadata)

        if set_template_values:
            # Note that retrieving schema requires internet access
            add_template_metadata_values(metadata, last_auto_index)
...
def add_template_metadata_values(metadata: CommentedMap, last_auto_index: int) -> None:
    ...
    # Move description to the end of auto-generated fields
    # and comment " ---- user please udpate ---- " before it
    metadata.insert(last_auto_index, "description", metadata.pop("description", placeholder_text))
...
def arrange_metadata(metadata) -> tuple[CommentedMap, int]:
        """Arrange metadata fields into auto-generated fields and user-editable fields with comments"""
        # Add comment "---- automated metadata fields ----" and insert auto-generated fields
        metadata.yaml_set_start_comment(DO_NOT_EDIT_COMMENT)

        metadata.insert(0, UUID_FIELD, metadata.pop(UUID_FIELD))
        last_auto_index, _ = move_one_metadata_field(metadata, NAME_FIELD, 1)
        
        # Start inserting may-edit fields
        may_edit_fields = [
            CONTACT_FIELD, EMAIL_FIELD, CREATED_FIELD, 
            GIT_URL_FIELD, MODEL_FIELD, PARENT_UUID_FIELD
        ]
        
        inserted_fields = []
        for field in may_edit_fields:
            last_auto_index, result = move_one_metadata_field(metadata, field, last_auto_index)
            if result:
                inserted_fields.append(field)

        # Add comment "---- automated fields but user may edit ----" in front of the may_edit fields 
        if inserted_fields:
            metadata.yaml_set_comment_before_after_key(
                inserted_fields[0],
                before=f"\n{CAN_EDIT_COMMENT}")
        
        return metadata, last_auto_index

def move_one_metadata_field(metadata, field_name, insert_index) -> tuple[int, bool]:
    """
    Move a metadata field to the designated index if this field exists and is not None.,
    Return the updated index and a boolean indicating if insert is successful
    """
    if field_name in metadata.keys() and metadata[field_name] is not None:
        metadata.insert(insert_index, field_name, metadata.pop(field_name))
        return insert_index + 1, True
